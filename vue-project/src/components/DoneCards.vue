...
const doneTaskArea = 'Done';

const doneTasks = computed(() => {
  return tasks.value ? tasks.value.filter(task => task.area === doneTaskArea) : [];
});
...
const handleUpdateTaskArea = async (taskId, task) => {
  try {
    ...
    task.area = task.is_complete ? 'Done' : 'To do';
...
<template>
  ...
  <div class="task-card" v-for="task in doneTasks" :key="task.id">
